  ...
  return (
    <div className='flex flex-col items-center px-6 md:px-16 lg:px-24 xl:px-32 pt-10 pb-20'>
      <div className='flex flex-col md:flex-row items-center justify-between w-full'>
        ...
        <button className='group flex items-center gap-2 front-medium cursor-pointer max-md:mt-12'>
          View All Offers
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-12'>
        {exclusiveOffers.map((item)=>(
          <div key={item._id} className='group relative flex flex-col items-start justify-between gap-1 pt-12 md:pt-18 px-4 rounded-xl text-white bg-no-repeat bg-cover bg-center' style={{backgroundImage: `url(${item.image})`}}>
            ...
            <button className='flex items-center gap-2 front-medium cursor-pointer mt-4 mb-5'>
              View Offers
